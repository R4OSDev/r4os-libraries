...
pub const Profile = struct {
    ...
    pub fn query(device: *d.Device) d.Error!Profile {
        ...
        return .{ .binding = selected.binding, .memory_generation = arch.memory_generation,
            .va_start = arch.va_start, .va_end = @min(arch.va_end, @as(u64,1) << 40), .graphics_class = arch.graphics_class };
    }
};
pub fn payload(value: anytype) bool { return value.version == 1 and value.size >= @sizeOf(@TypeOf(value)); }
fn valid(value: a.GfxBufferHandle) bool { return value.id != 0 and value.generation != 0 and value.reserved0 == 0; }
fn equal(left: anytype, right: @TypeOf(left)) bool { return std.meta.eql(left, right); }
...
pub const Resource = struct {
    ...
    pub fn live(self: *const Resource) bool { return self.backing.reference.id != 0 or self.range.id != 0 or self.map.lease.id != 0; }
    ...
    pub fn system(self: *Resource, device: *d.Device, profile: Profile, deadline: u64) d.Error!void {
        ...
        self.deadline = deadline;
        try d.platform(device.buffers().create(&.{ .byte_length = granule, .alignment = granule,
            .usage = a.gfx_buffer_usage_cpu_read | a.gfx_buffer_usage_cpu_write | a.gfx_buffer_usage_transfer_source | a.gfx_buffer_usage_transfer_target }, &self.backing));
        try self.describe(device, profile);
    ...
    fn describe(self: *Resource, device: *d.Device, profile: Profile) d.Error!void {
        ...
        if (desc.modifier != 0 and (desc.modifier & ~@as(u64,15) != modifier_base or desc.modifier & 15 > 5)) return error.Unsupported;
    ...
    pub fn step(self: *Resource, device: *d.Device, profile: Profile) d.Error!bool {
        ...
        if (!valid(self.range)) {
            ...
            try d.platform(base.gfxVirtualStart(&.{ .kind = 1, .adapter_id = profile.binding.adapter_id,
                .memory_generation = profile.memory_generation, .deadline_ns = self.deadline,
                .byte_length = self.descriptor.byte_length, .alignment = granule,
                .location = @intFromBool(self.descriptor.location == a.gfx_buffer_location_device_local),
                .flags = if (self.descriptor.modifier != 0) a.gfx_virtual_flag_blocklinear | (6 << a.gfx_virtual_layout_shift) else 0 }, &result));
        ...
        if (!valid(self.binding)) {
            ...
            var result: a.GfxVirtualStatus = .{};
            try d.platform(base.gfxVirtualStart(&.{ .kind = 2, .adapter_id = profile.binding.adapter_id,
                .memory_generation = profile.memory_generation, .parent = self.range, .reference = self.backing.reference,
                .byte_length = self.descriptor.byte_length, .deadline_ns = self.deadline }, &result));
    ...
    pub fn close(self: *Resource, device: *d.Device) d.Error!void {
        ...
        if (valid(self.range)) {
            ...
            self.range = .{}; self.binding = .{};
        }
        if (valid(self.map.lease)) { try d.platform(device.buffers().unmap(&self.map.lease)); self.map = .{}; }
        if (valid(self.backing.reference)) { try d.platform(device.buffers().release(&self.backing.reference)); self.backing = .{}; }
        self.* = .{};
    }
};
