//! Persistent normalized panel brightness, keyed by exact stable receiver identity.
const std = @import("std");
const topology = @import("topology.zig");
pub const path = "C:\\R4OS\\CONFIG\\BRIGHTNESS.R4S";
...
pub const Choice = struct {
    ...
    pub fn validate(self: Choice) Error!void {
        if (!self.key.persistable() or self.level > 65535 or self.serial == 0) return error.Invalid;
    }
};
pub const Config = struct {
    ...
    pub fn change(self: *const Config, key: topology.Key, level: u32) Error!Config {
        ...
        var next = self.*;
        const choice: Choice = .{ .key = key, .level = level, .serial = serial };
        try choice.validate();
    ...
    pub fn parse(bytes: []const u8) Error!Config {
        ...
        while (lines.next()) |raw| {
            ...
            if (std.mem.eql(u8, line, "SCHEMA=GFX_BRIGHTNESS_1")) { if (schema) return error.Format; schema = true; continue; }
            ...
            choice.level = try number(u32, &fields, 10);
    ...
    pub fn encode(self: *const Config, buffer: []u8) Error![]const u8 {
        ...
        for (self.choices[0..self.count], 0..) |choice, i| {
            ...
            const line = std.fmt.bufPrint(buffer[used..], "DISPLAY={x:0>16},{d},{s},{d},{d}\r\n",
                .{choice.key.adapter, choice.key.connector, receiver, choice.level, choice.serial}) catch return error.Buffer;
            used += line.len;
        }
