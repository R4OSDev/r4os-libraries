...
pub const Worker = struct {
    ...
    pub fn start(self: *Worker, request: c.R4AcoRequest) i32 {
        if (self.handle.thread_id != 0 or self.live_bytes != 0) return c.status_busy;
        self.request = request;
        self.result = std.mem.zeroes(c.R4AcoBinary);
        self.cancel_requested.store(false, .release);
        if (self.sys.threadCreateHandle(entry, @intFromPtr(self), 2 * 1024 * 1024, 0, &self.handle) != a.thread_ok) return c.status_memory;
        return c.status_ok;
    }
    ...
    fn entry(user: u64) callconv(.c) i32 {
        const self = at(user);
        const runtime: c.R4AcoRuntime = .{
            .version = 1,
            .size = @sizeOf(c.R4AcoRuntime),
            .user = user,
...
pub fn loadCache(sys: *const r4os.r4sys.Context, client: *const c.CompilerV1Client, path: [*:0]const u8, key: *const c.R4AcoCacheKey, output: *c.R4AcoBinary, code: []u8) i32 {
...
pub fn saveCache(sys: *const r4os.r4sys.Context, client: *const c.CompilerV1Client, path: [*:0]const u8, staging: [*:0]const u8, backup: [*:0]const u8, key: *const c.R4AcoCacheKey, binary: *const c.R4AcoBinary, code: []const u8) i32 {
