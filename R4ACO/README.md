...
R4ACO is the AMD shader compiler library owner. Version 0.1.0 provides a generated
...
Build from the Libraries repository with `./Build.sh R4ACO` on Linux or
`Build.bat R4ACO` on Windows. Both use PowerShell 7 and the workspace
`Settings.R4S`. The normal build runs C/Zig ABI conformance and translates
real Mesa 26.2.2 ACO assembler/liveness and a NIR optimization pass sources for
...
0.80.13; see `Docs/Drivers/AMDModulgrundlage08002.txt` in the workspace.
