...
pub const Layout = struct {
    ...
    pub fn init(width: u32, height: u32) Error!Layout {
        ...
        const luma_bytes = @as(usize, pitch) * height;
        return .{ .width = width, .height = height, .pitch = pitch, .uv_offset = luma_bytes,
            .bytes = std.mem.alignForward(usize, luma_bytes + @as(usize, pitch) * (height / 2), 65536) };
    }
};
fn rgb(pixel: u32) [3]i64 { return .{ @intCast((pixel >> 16) & 255), @intCast((pixel >> 8) & 255), @intCast(pixel & 255) }; }
...
pub fn rows(source: []const u32, stride: u32, layout: Layout, output: []u8, first: u32, count: u32) Error!void {
    const checked = try Layout.init(layout.width, layout.height);
    ...
    while (y < first + count) : (y += 2) {
        ...
        for (row0, row1, 0..) |a, b, x| { out0[x] = luma(a); out1[x] = luma(b); }
        @memset(out0[layout.width..], 16); @memset(out1[layout.width..], 16);
        ...
        while (x < layout.width) : (x += 2) {
            ...
            for ([_][]const u32{ row0, row1 }) |row| {
                const l = rgb(row[left]); const center = rgb(row[x]); const rr = rgb(row[right]);
