...
pub export fn r4vk_window_request(raw: *const a.R4XStartContext,
    request: *const a.WindowGraphicsRequest, output: *a.WindowGraphicsReply) callconv(.c) bool
{
...
pub export fn r4vk_window_service_dead(raw: *const a.R4XStartContext,
...
pub export fn r4vk_window_wait(raw: *const a.R4XStartContext,
    owner: *const a.ProgramProcessHandle, revision: u64, duration_ns: u64) callconv(.c) void
{
    ...
    defer _ = connection.close();
    const request: a.WindowGraphicsWait = .{ .owner = owner.*, .known_revision = revision, .deadline_tick = sys.ticks() +| @max(ticks, 1) };
    const result = connection.callTyped(a.WindowGraphicsWait, a.WindowGraphicsReply, a.window_graphics_op_wait, &request, timeout);
    switch (result) {
        .timed_out => {},
