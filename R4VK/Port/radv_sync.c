...
struct native_sync {
   ...
   struct r4vk_radv_point *point;
   bool signaled;
   bool hold_fence;
};
static struct native_sync *native(struct vk_sync *base)
{
   assert(base->type == &r4vk_radv_sync_type);
   return container_of(base, struct native_sync, base);
}
bool r4vk_radv_sync_supported(const struct vk_sync *sync, uint64_t value)
{
   return sync && sync->type == &r4vk_radv_sync_type && !sync->flags && !value;
}
...
static void sync_finish(struct vk_device *device, struct vk_sync *base)
{
   (void)device;
   struct native_sync *sync = native(base);
   if (sync->hold_fence && sync->point) r4vk_radv_point_unexport(sync->point);
   r4vk_radv_point_unref(sync->point);
                         ...
                         struct r4vk_radv_ws *resources)
{
   while (!sync->signaled && !sync->point) {
      if ((sync->device && vk_device_is_lost_no_report(sync->device)) ||
          (resources && r4vk_radv_is_lost(resources)))
...
VkResult r4vk_radv_sync_point(struct vk_sync *base,
                            struct r4vk_radv_ws *resources,
                            struct r4vk_radv_point **out)
{
   struct native_sync *sync = native(base);
   VkResult result = lock(sync);
   if (result != VK_SUCCESS) return result;
   result = pending(sync, UINT64_MAX, resources);
   if (result == VK_SUCCESS) { *out = sync->point; r4vk_radv_point_ref(*out); }
   unlock(sync);
   return result;
}
static VkResult replace(struct native_sync *sync, struct r4vk_radv_point *point,
                          bool signaled)
{
   VkResult result = lock(sync);
   if (result != VK_SUCCESS) return result;
   struct r4vk_radv_point *old = sync->point;
   const bool old_pinned = sync->hold_fence && old;
   if (sync->hold_fence && point) {
      result = r4vk_radv_point_pin(point, NULL);
      if (result != VK_SUCCESS) { unlock(sync); return result; }
   }
   r4vk_radv_point_ref(point);
   sync->point = point;
   sync->signaled = signaled;
   if (u_cnd_monotonic_broadcast(&sync->condition) != thrd_success)
      result = VK_ERROR_DEVICE_LOST;
   unlock(sync);
   if (old_pinned) r4vk_radv_point_unexport(old);
   r4vk_radv_point_unref(old);
   return result;
}
VkResult r4vk_radv_sync_assign(struct vk_sync *base, struct r4vk_radv_point *point)
{
   assert(point);
   return replace(native(base), point, false);
}
VkResult r4vk_radv_sync_prepare_present(struct vk_sync *base)
{
   if (!r4vk_radv_sync_supported(base, 0)) return VK_ERROR_FEATURE_NOT_PRESENT;
   struct native_sync *sync = native(base);
   VkResult result = lock(sync);
   if (result != VK_SUCCESS) return result;
...
VkResult r4vk_radv_sync_take_present(struct vk_sync *base,
   struct r4vk_radv_point **out, R4GfxFence *fence)
{
   struct native_sync *sync = native(base);
   VkResult result = lock(sync);
   if (result != VK_SUCCESS) return result;
   ...
   if (result == VK_SUCCESS) {
      ...
      result = r4vk_radv_point_pin(sync->point, fence);
      if (result == VK_SUCCESS) {
         r4vk_radv_point_unexport(sync->point);
         r4vk_radv_point_ref(sync->point);
         *out = sync->point;
         sync->hold_fence = false;
      }
   }
...
{
   (void)device;
   if (!r4vk_radv_sync_supported(base, value)) return VK_ERROR_FEATURE_NOT_PRESENT;
...
{
   (void)device;
   if (dst_base == src_base || !r4vk_radv_sync_supported(dst_base, 0) ||
       !r4vk_radv_sync_supported(src_base, 0)) return VK_ERROR_UNKNOWN;
   ...
   struct r4vk_radv_point *old = dst->point;
...
{
   (void)device;
   if (!r4vk_radv_sync_supported(base, value) ||
   ...
   if (result == VK_SUCCESS && !(flags & VK_SYNC_WAIT_PENDING)) {
      point = sync->point; r4vk_radv_point_ref(point);
   }
   unlock(sync);
   if (point) {
      result = r4vk_radv_point_wait(point, until);
      r4vk_radv_point_unref(point);
   }
   return result;
}

const struct vk_sync_type r4vk_radv_sync_type = {
