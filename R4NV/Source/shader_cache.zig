...
const generated = @import("Generated/Shaders/shaders.zig");
...
comptime {
    if (generated.compiler_abi != c.shader_abi or generated.resource_abi != c.shader_resource_abi)
        @compileError("Regenerate the fixed shaders for the current shader/resource ABI");
    for (&generated.programs) |program| {
...
fn programFor(profile: u32) ?*const generated.Program {
    for (&generated.programs) |*program| if (program.profile == profile) return program;
    return null;
}
...
fn validateKey(key: *const c.R4NvShaderKey) i32 {
    ...
    if (key.vendor_id != 0x10de or key.graphics_class != c.shader_graphics_class_ampere_b or key.shader_model != c.shader_model_sm86 or
...
fn infoFor(program: *const generated.Program) c.R4NvShaderInfo {
    return .{ .version = 1, .size = @sizeOf(c.R4NvShaderInfo), .profile = program.profile, .stage = program.stage,
        .shader_model = c.shader_model_sm86, .registers = program.gprs, .code_bytes = @intCast(program.code.len),
        .instructions = program.instructions, .scratch_bytes = 0, .stack_bytes = 0, .header_bytes = 128,
        .resource_abi = generated.resource_abi, .max_warps_per_sm = program.max_warps_per_sm, .reserved = 0,
        .compiler_id = digestValue(&generated.compiler_id) };
}
...
fn prefix(program: *const generated.Program, key: *const c.R4NvShaderKey) [prefix_bytes]u8 {
    ...
    @memcpy(bytes[32..64], &generated.compiler_id);
    ...
    for ([_]u32{ program.stage, c.shader_model_sm86, program.gprs, program.instructions,
        program.max_warps_per_sm, 0, 0, generated.resource_abi }, 0..) |value, i| put(u32, &bytes, 192 + i * 4, value);
...
pub fn r4nv_shader_info_impl(profile: u32, output: *c.R4NvShaderInfo) callconv(.c) i32 {
    ...
    const program = programFor(profile) orelse return c.status_unsupported;
    output.* = infoFor(program);
    return c.status_ok;
}
...
pub fn r4nv_shader_cache_read_impl(key: *const c.R4NvShaderKey, bytes: [*]const u8, length: u32, output: *c.R4NvShaderView) callconv(.c) i32 {
    ...
    const program = programFor(profile) orelse return c.status_cache_miss;
    if (length != code_offset + program.code.len) return c.status_cache_miss;
    const expected = prefix(program, key);
    ...
    output.* = .{ .info = infoFor(program), .header_address = @intFromPtr(bytes + header_offset), .code_address = @intFromPtr(bytes + code_offset) };
    return c.status_ok;
}
