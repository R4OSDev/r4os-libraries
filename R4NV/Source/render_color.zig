//! Fixed userland color-program upload, matching COLOR fragment CBuf2.
...
pub const Program = extern struct {
    ...
    pub fn validate(self: Program) error{ Bounds, Unsupported }!void {
        ...
        for ([_]usize{128,144}) |byte| {
            const white = self.scalar(byte); const peak = self.scalar(byte + 4);
            const gamma = self.scalar(byte + 8); const beta = self.scalar(byte + 12);
