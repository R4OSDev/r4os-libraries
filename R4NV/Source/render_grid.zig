...
pub const Grid = extern struct {
    enabled: u32 = 0, rotation: u32 = 0, scale: u32 = 0, pixel_width: u32 = 0,
    pixel_height: u32 = 0, target_x: i32 = 0, target_y: i32 = 0, reserved: u32 = 0,
    viewport_x: i32 = 0, viewport_y: i32 = 0, viewport_width: u32 = 0, viewport_height: u32 = 0,
    guest_width: u32 = 0, guest_height: u32 = 0, source_x: u32 = 0, source_y: u32 = 0,
    ...
    pub fn sample(self: Grid, x: i64, y: i64) error{Bounds}![2]u32 {
        const px = x + self.target_x; const py = y + self.target_y;
        ...
        const oriented: [2]i64 = switch (self.rotation) {
            0 => .{ px, py }, 1 => .{ self.pixel_height - 1 - py, px },
            2 => .{ self.pixel_width - 1 - px, self.pixel_height - 1 - py },
            3 => .{ py, self.pixel_width - 1 - px }, else => return error.Bounds,
        };
...
comptime { if (@sizeOf(Grid) != 64) @compileError("fixed grid descriptor"); }
